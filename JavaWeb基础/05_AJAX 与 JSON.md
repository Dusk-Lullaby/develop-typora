...
#### xxxxxxxxxx59 1package com.sonnet.excel;2​3import com.alibaba.excel.EasyExcel;4import com.alibaba.excel.ExcelWriter;5import com.alibaba.excel.context.AnalysisContext;6import com.alibaba.excel.read.listener.ReadListener;7import com.alibaba.excel.write.metadata.WriteSheet;8​9import java.io.*;10import java.util.ArrayList;11import java.util.List;12​13public class ExcelUtil {14​15// 每个sheet中写的最大数据16private static final int MAX_COUNT_PER_SHEET = 5000;17​18    public static <T> List<T> readExcel(InputStream inputStream, Class<T> clazz) {19        List<T> dataList = new ArrayList<>();20        ReadListener<T> readListener = new ReadListener<T>() {21            @Override22            public void invoke(T t, AnalysisContext analysisContext) {23                dataList.add(t);24            }25​26            @Override27            public void doAfterAllAnalysed(AnalysisContext analysisContext) {28            }29        };30        EasyExcel.read(inputStream, clazz, readListener)31                .doReadAll();32        return dataList;33    }34​35    public static <T> void writerExcel(OutputStream outputStream, Class<T> clazz, String sheetName, List<T> dataList) {36        // EasyExcel写excel时必须指定excel存放位置，37        // 还需要指定写的时候excel的表头与属性的对应关系38        ExcelWriter writer = EasyExcel.write(outputStream, clazz).build();39        // 数据总条数40        int size = dataList.size();41        // 计算sheet数量42        // 例如：10000条数据，每个Sheet写5000条，则需要2个Sheet43        int sheetCount = (size + MAX_COUNT_PER_SHEET - 1) / MAX_COUNT_PER_SHEET;44        if (sheetCount == 0) {45            sheetCount++;46        }47        for (int i = 0; i < sheetCount; i++) {48            int start = i * MAX_COUNT_PER_SHEET;49            int end = (i + 1) *  MAX_COUNT_PER_SHEET;50            end = Math.min(end, size);51            List<T> sheetData = dataList.subList(start, end);52            WriteSheet writeSheet = new WriteSheet();53            writeSheet.setSheetNo(i);54            writeSheet.setSheetName(sheetName + (i + 1));55            writer.write(sheetData, writeSheet);56        }57        writer.finish();58    }59}java
